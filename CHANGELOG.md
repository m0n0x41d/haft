...
- **Harness batch drain workflow** (`dec-20260428-harness-drain-v3-16bf21f3`) — opt-in `haft harness run --drain --concurrency N` operator path drains runnable WorkCommissions while preserving claim-time lockset conflicts and AutonomyEnvelope preflight/execute gates. Per-commission `delivery_policy` is the apply-authority gate (V3 invariant): a terminal commission with verdict=pass and `delivery_policy=workspace_patch_auto_on_pass` triggers a discrete local auto-apply directly from the drain loop, while `workspace_patch_manual` (default) keeps the diff in the workspace clone awaiting `haft harness apply`. AutonomyEnvelope is consulted at creation/preflight/execute as before; an explicitly blocked envelope still keeps the manual path even on auto_on_pass policy. Stale leases older than the configured age cap (default 24h) are skipped at intake with a typed `lease_too_old` reason so old claimed commissions require explicit cancel or requeue instead of silent revival. Three slices: `f28a6615` drain flag + Open-Sleigh keep-alive, `48593198` per-commission delivery decision, `61874c5c` stale-lease age cap; final auto-apply trigger wires the decision into the drain loop.
