...
func TestAttachEvidence_AcceptsEquivalentClaimRefsAndScopeInDifferentOrders(t *testing.T) {
	store := setupTestDB(t)
	ctx := context.Background()
	haftDir := t.TempDir()
	...
	dec, _, _ := Decide(ctx, store, haftDir, completeDecision(DecideInput{
		SelectedTitle: "Test",
		WhySelected:   "Equivalent claim bindings should not be rejected because of ordering.",
		Predictions: []PredictionInput{
			{
				Claim:      "Latency stays under 50ms",
				Observable: "latency",
	...
	if got := strings.Join(item.ClaimRefs, ","); got != "claim-002,claim-001" {
		t.Fatalf("claim_refs = %q, want explicit ref order preserved", got)
	}
	if got := strings.Join(item.ClaimScope, ","); got != "Latency stays under 50ms,Throughput stays above 100k events/sec" {
		t.Fatalf("claim_scope = %q, want normalized equivalent scope", got)
	}
}
