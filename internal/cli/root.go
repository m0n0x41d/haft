...
Haft is consumed via three surfaces sharing one artifact graph:
  - Skills in your AI coding agent (Claude Code, Codex, etc.)
...
Examples:
  ...
  haft init --codex                 # explicit host init for scripts and CI
  haft serve                        # start the MCP server for other agents
  haft fpf lookup "B.5.2" --json    # exact-first FPF source lookup
