...
func TestTransportActionParity(t *testing.T) {
	...
	knownTransportDrift := map[string]map[string]string{
		...
		"haft_query": {
			...
			"contract_generation":  "MCP-only — explicit read-only contract generation manifest; standalone has CLI `haft interface contract-generation` instead of the old standalone tool transport",
			...
			"change_case":          "MCP-only — explicit read-only EngineeringChangeCase projection; standalone has CLI `haft change case` instead of the old standalone tool transport",
			...
			"drift_events":         "MCP-only — explicit read-only DriftEvent aggregation; standalone has CLI `haft drift events` instead of the old standalone tool transport",
			"decision_reconcile":   "MCP-only — explicit read-only DecisionReconciliationPlan; standalone has CLI `haft decision reconcile` instead of the old standalone tool transport",
			"governing_set":        "MCP-only — explicit read-only CurrentGoverningSet; standalone has CLI `haft decision governing-set` instead of the old standalone tool transport",
