...
func disableEcho(fd uintptr) (restore func(), err error) {
	...
	oldTermios, err := unix.IoctlGetTermios(fdInt, unix.TIOCGETA)
	if err != nil {
		return nil, err
	}
	modified := *oldTermios
	modified.Lflag &^= unix.ECHO | unix.ECHOE | unix.ECHOK | unix.ECHONL
	if err := unix.IoctlSetTermios(fdInt, unix.TIOCSETA, &modified); err != nil {
		return nil, err
	}
	return func() {
		_ = unix.IoctlSetTermios(fdInt, unix.TIOCSETA, oldTermios)
	}, nil
}
