...
func haftCommissionTool() Tool {
	return Tool{
		...
		Description: "Create, list, show, claim, requeue, cancel, and update WorkCommissions. Creation actions are binding governance acts; MCP fails them closed by default with operator_confirmation_required because model-supplied arguments are not kernel-verifiable authorization receipts. Manual CLI remains the default binding path; host receipts require a registered kernel verifier. WorkCommissions are bounded execution authorizations between DecisionRecords and RuntimeRuns; external trackers are optional projections, not work authority.",
