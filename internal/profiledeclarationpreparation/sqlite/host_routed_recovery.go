...
func recoverExistingHostRoutedAuthority(
	ctx context.Context,
	...
	if basisFound != resolutionFound {
		return existingAuthority{}, authorityConflict,
			"host-routed profile authority basis and resolution are not an atomic pair", nil
	}
	...
	if err != nil {
		return existingAuthority{}, authorityConflict,
			fmt.Sprintf("host-routed profile authority checked_at is invalid: %v", err), nil
	}
	...
	if string(durableInput.CanonicalJSON()) != string(plan.Input().CanonicalJSON()) {
		return existingAuthority{}, authorityConflict,
			"durable host-routed authority binds another reviewed Work input", nil
	}
	bindingDigest, err := loadProjectBindingDigest(ctx, transaction, plan.Root())
	if err != nil {
		return existingAuthority{}, authorityAbsent, "", err
	}
	expectedBasis, expectedResolution, err := buildHostRoutedAuthorityRows(plan, bindingDigest)
	if err != nil {
		return existingAuthority{}, authorityConflict, err.Error(), nil
	}
	checks := []struct {
	...
	for _, check := range checks {
		if !check.matches {
			return existingAuthority{}, authorityConflict,
				"existing host-routed profile authority differs at " + check.name, nil
		}
	}
	return existingAuthority{
		plan: plan, basisDigest: expectedBasis.digest,
