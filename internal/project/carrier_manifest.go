	...
	entries := []CarrierManifestEntry{
		{
			...
			Notes:          "target/software/term-map carriers are reviewable projections imported through explicit sync/lifecycle commands",
		},
		{
		...
		{
			ID:             "fpf-readme-source",
			PathPattern:    "data/FPF/Readme.md",
			AuthorityClass: CarrierAuthorityCurrent,
			Surface:        "fpf_source",
			Current:        true,
			Normativity:    "upstream authored FPF practical-use and publication source",
			Notes:          "FPF Query results retain the source commit, path, span, and body hash",
		},
		{
			ID:             "fpf-spec-source",
			PathPattern:    "data/FPF/FPF-Spec.md",
			AuthorityClass: CarrierAuthorityCurrent,
			Surface:        "fpf_source",
			Current:        true,
			Normativity:    "upstream authored FPF specification source",
			Notes:          "FPF Query results retain the source commit, path, span, and body hash",
		},
		{
			ID:             "fpf-query-index",
			PathPattern:    "internal/cli/fpf.db",
			AuthorityClass: CarrierAuthoritySupport,
			Surface:        "retrieval_cache",
			Current:        true,
			Normativity:    "derived non-normative source index; the upstream Markdown remains authoritative",
			Notes:          "rebuild atomically from one verified FPF source commit; never treat ranking as pattern selection",
		},
		{
		...
		},
		{
			...
			DeadSurfacePolicy: "standalone agent, its coding-agent TUI, and desktop wrappers may appear only as dropped/archive/provenance surfaces",
		},
		{
		...
		{
			ID:                "desktop-standalone-code",
			PathPattern:       "desktop/**",
			AuthorityClass:    CarrierAuthorityArchive,
			Surface:           "dead_runtime_surface",
			Current:           false,
			Normativity:       "dropped desktop and standalone-wrapper surface; retained only as archive/provenance/support unless explicitly reopened",
			DeadSurfacePolicy: "do not present the standalone agent or desktop wrapper as current product surfaces",
		},
		{
