...
const (
	noteSignatureID       = "Haft.NoteAtConcern"
	noteSlotID            = "Haft.NoteAtConcern.NoteSlot"
	entityOfConcernSlotID = "Haft.NoteAtConcern.EntityOfConcernSlot"
...
func TestAdaptBuildsGenericRecordAndNoteAtExactConcern(t *testing.T) {
	...
	if declaration.Entity() != fixture.draft.RecordEntity() {
		t.Fatal("Note adapter declared a different project-record identity")
	}
	...
	if relation.Signature().ID().String() != noteSignatureID {
		t.Fatalf("relation signature = %q, want %q", relation.Signature().ID(), noteSignatureID)
	}
	...
	if candidate.MappingManifestRef() != manifest.Ref() ||
		...
		t.Fatal("record-carrier binding lost the exact Note mapping coordinate")
	}
...
func TestAdaptCanonicalizesClaimGraphPermutation(t *testing.T) {
	...
	if forwardDigest != reverseDigest {
		t.Fatal("ClaimGraph node permutation changed Note candidate identity")
	}
}
...
func TestAdaptKeepsMissingConcernAndMappingBasisUnderdetermined(t *testing.T) {
	...
	missingRelationEnvironment := environmentWithoutNoteRelation(t, fixture.environment)
	...
	assertUnderdetermined(
		...
		"note_typed_relation_declaration_fragment",
	)
}
...
func newAdapterFixture(t *testing.T) adapterFixture {
	...
	relation := testNoteRelationSignature(
		t,
		ref,
		context,
	...
	environment, err := typedmemory.NewTypeEnvBuilder(ref).
		...
		Build()
	if err != nil {
		t.Fatalf("build Note adapter TypeEnv fixture: %v", err)
	}
	...
	recordEntity := mustValueTest(typedmemory.NewEntityID("record:note-1"))
	draft := mustValueTest(NewDraft(DraftInput{
		ProjectID:      project,
		RecordEntity:   recordEntity,
		RecordLocalRef: mustValueTest(typedmemory.NewBatchLocalRef("record:note-1")),
		RecordLabel:    mustValueTest(typedmemory.NewEntityLabel("Note: retain CAS evidence")),
		AssertionID:    mustValueTest(typedmemory.NewAssertionID("assertion:note-1-at-concern")),
		ContextSlice:   contextSlice,
		ClaimGraph: mustValueTest(
			NewExactClaimGraph(testClaimGraph(t, textValueKind, false)),
		),
		Provenance: mustValueTest(typedmemory.NewProvenanceRef("memory:note-adapter-test")),
	}))
...
func testRegistrationPolicy(
	...
	artifact := mustValueTest(typedmemory.NewCarrierRef(
		"artifact:note-adapter-membership-runtime/v1",
	))
...
func testNoteRelationSignature(
	t *testing.T,
	...
	t.Helper()
	signatureID := mustValueTest(typedmemory.NewSignatureID(noteSignatureID))
	...
	slots := []typedmemory.SlotSpec{
		mustValueTest(typedmemory.NewSlotSpec(
			mustValueTest(typedmemory.NewSlotKindID(noteSlotID)),
			noteTarget,
...
func environmentWithoutNoteRelation(
	t *testing.T,
...
func testDeclarationProvenance(t *testing.T) typedmemory.FPFSourceProvenance {
	...
	reference := mustValueTest(typedmemory.NewProvenanceRef("provenance:note-adapter-fixture"))
	rule := mustValueTest(typedmemory.NewCompilerRuleID("fixture.note-adapter.v1"))
...
func testSourceLocation(t *testing.T) typedmemory.SourceLocation {
	t.Helper()
	unit := mustValueTest(typedmemory.NewSourceUnitID("source:note-adapter-fixture"))
	revision := mustValueTest(typedmemory.NewSourceRevision("fixture-revision-1"))
...
func mustValueTest[T any](value T, err error) T {
	if err != nil {
		panic("Note adapter fixture construction failed: " + err.Error())
	}
	return value
}
