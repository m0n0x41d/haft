// Package problemcardadapter owns the sealed mapping coordinates for the
// ProblemCardAtConcern task adapter. The manifest is not a TypeEnv, admission
// decision, storage effect, or authority grant.
...
const (
	...
	MappingManifestIDV1      = "haft.problem-card-at-concern"
	MappingManifestVersionV1 = "2.0.0"
	AdapterVersionV1         = "haft-problem-card-adapter/2.0.0"
)
...
// ProblemCard task adapter. Its content-derived identity is the exact policy
// coordinate intended for a record-carrier binding; it is not admission
// authority.
...
func CurrentMappingManifestV1() (MappingManifestV1, error) {
	...
	if err != nil {
		return MappingManifestV1{}, fmt.Errorf(
			"encode ProblemCard mapping manifest: %w",
			err,
		)
	}
...
func decodeMappingManifestV1(canonical []byte) (MappingManifestV1, error) {
	...
	if err := decoder.Decode(&encoded); err != nil {
		return MappingManifestV1{}, fmt.Errorf(
			"decode ProblemCard mapping manifest: %w",
			err,
		)
	}
	...
	if err != nil {
		return MappingManifestV1{}, fmt.Errorf(
			"encode expected ProblemCard mapping manifest: %w",
			err,
		)
	}
	if !bytes.Equal(canonical, expectedBytes) {
		return MappingManifestV1{}, fmt.Errorf(
			"ProblemCard mapping manifest is unsupported or not canonical",
		)
	}
	...
	if err != nil {
		return MappingManifestV1{}, fmt.Errorf(
			"derive ProblemCard mapping manifest digest: %w",
			err,
		)
	}
	...
	if err != nil {
		return MappingManifestV1{}, fmt.Errorf(
			"derive ProblemCard mapping manifest reference: %w",
			err,
		)
	}
	...
	if err != nil {
		return MappingManifestV1{}, fmt.Errorf(
			"derive ProblemCard adapter version: %w",
			err,
		)
	}
...
func (manifest MappingManifestV1) Verify() error {
	...
	if decoded.ref != manifest.ref || decoded.adapter != manifest.adapter {
		return fmt.Errorf(
			"ProblemCard mapping manifest identity does not match canonical bytes",
		)
	}
	return nil
}
...
func canonicalMappingManifestV1() mappingManifestCanonicalV1 {
	return mappingManifestCanonicalV1{
		...
		EmittedSignature: "Haft.ProblemCardAtConcern",
		EmittedSlots: []string{
		...
		RoundTripFixtures: []string{
			"valid_generic_problem_card_at_exact_concern",
...
func requireJSONEnd(decoder *json.Decoder) error {
	...
	if err != nil {
		return fmt.Errorf(
			"decode trailing ProblemCard mapping manifest content: %w",
			err,
		)
	}
	return fmt.Errorf("ProblemCard mapping manifest contains trailing JSON content")
}
