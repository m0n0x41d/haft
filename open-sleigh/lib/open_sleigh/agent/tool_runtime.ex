defmodule OpenSleigh.Agent.ToolRuntime do
  ...
  * `bash` is authorized only as the `:run_tests` action before execution. Arbitrary
  ...
  defp reject_commission_lifecycle_mutation(command) do
    command
    |> commission_lifecycle_mutation_command?()
    |> commission_lifecycle_mutation_result()
  end

  @spec commission_lifecycle_mutation_result(boolean()) :: :ok | {:error, EffectError.t()}
  defp commission_lifecycle_mutation_result(true), do: {:error, :no_commission_mutation}
  defp commission_lifecycle_mutation_result(false), do: :ok
  ...
  defp commission_lifecycle_mutation_command?(command) do
    ...
    commission_lifecycle_mutation_patterns()
    |> Enum.any?(&Regex.match?(&1, normalized))
  end
