...
func setupAssuranceTestEnv(t *testing.T) (*main.FSM, *db.DB, string) {
	tempDir := t.TempDir()
	...
	fsm := &main.FSM{
		State: main.State{Phase: main.PhaseDecision},
...
func TestAssuranceGuard_BlocksLowR(t *testing.T) {
	...
	ra := main.RoleAssignment{Role: main.RoleDecider, SessionID: "test", Context: "test"}
	ev := &main.EvidenceStub{URI: l2File, Type: "hypothesis", HolonID: "test-holon"}

	ok, msg := fsm.CanTransition(main.PhaseOperation, ra, ev)
...
func TestAssuranceGuard_AllowsHighR(t *testing.T) {
	...
	ra := main.RoleAssignment{Role: main.RoleDecider, SessionID: "test", Context: "test"}
	ev := &main.EvidenceStub{URI: l2File, Type: "hypothesis", HolonID: "good-holon"}
...
func TestAssuranceGuard_RespectsConfigurableThreshold(t *testing.T) {
	...
	ra := main.RoleAssignment{Role: main.RoleDecider, SessionID: "test", Context: "test"}
	ev := &main.EvidenceStub{URI: l2File, Type: "hypothesis", HolonID: "medium-holon"}

	// With default threshold (0.8), should be blocked
	ok, _ := fsm.CanTransition(main.PhaseOperation, ra, ev)
	if ok {
...
func TestAuditVisualization_ReturnsTree(t *testing.T) {
	...
	fsm, _ := main.LoadState(stateFile, rawDB)
