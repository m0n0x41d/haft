...
Before calling `quint_decision`, run a verification check. The agent that generates a decision cannot be its sole validator (FPF A.12 — External Transformer Principle).
...
>
> Proceed? (If the counter-argument kills the decision → go back to /q-explore)
...
After presenting: if the user confirms, record the decision. If any probe kills the decision, go back to /q-explore.
