...
// ---------------------------------------------------------------------------
// Auth storage — ~/.config/quint-code/auth.json
// ---------------------------------------------------------------------------
...
func quintAuthPath() string {
	home, _ := os.UserHomeDir()
	return filepath.Join(home, ".config", "quint-code", "auth.json")
}

func loadQuintAuth() *quintAuth {
	data, err := os.ReadFile(quintAuthPath())
	if err != nil {
		return nil
	}
	var auth quintAuth
...
func saveQuintAuth(auth *quintAuth) error {
	path := quintAuthPath()
