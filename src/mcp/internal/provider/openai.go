...
// Auth resolution: env var -> quint auth (codex oauth / api key) -> codex CLI token.
...
func buildResponseParams(
	...
	if authType == "api_key" {
		params.MaxOutputTokens = openai.Int(16384)
	}
	if instructions != "" {
...
func textInput(text string) responses.ResponseInputMessageContentListParam {
	if text == "" {
		return nil
	}
...
func writeDebugLog(err error, requestDebug string) {
	...
	logDir := filepath.Join(home, ".config", "quint-code")
...
func resolveAuth() resolvedAuth {
	...
	// 2. Quint's own auth file (from `quint-code login`)
	if auth := loadQuintAuthFile(); auth != nil {
...
func loadQuintAuthFile() *quintAuthFile {
	home, err := os.UserHomeDir()
	if err != nil {
		return nil
	}
	data, err := os.ReadFile(filepath.Join(home, ".config", "quint-code", "auth.json"))
	if err != nil {
		return nil
	}
	var auth quintAuthFile
...
func refreshCodexToken(refreshToken string) *resolvedAuth {
	...
	if auth != nil {
		...
		path := filepath.Join(home, ".config", "quint-code", "auth.json")
